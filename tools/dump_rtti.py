...
Usage:
    python tools/dump_rtti.py [path_to_pivot.exe] [--out DIR]
        [--json] [--markdown] [--all-classes]

The default mode prints a text listing to stdout. With --json, --markdown, or
--all-classes it writes to docs/reference unless --out is supplied:
...
# Anchors from Pivot 5.2.11; these mirror src/pivotkit.cpp.
IMAGEBASE = 0x400000
VMT_OFFSET = 12
...
# Group names by functional area for the human-readable catalog.
...
def main():
    ...
    ap.add_argument("--out", default=None, help="output directory for generated reference files")
    ...
    # Find TMainForm TypeInfo using the same image scan as the native loader.
    ...
    # Write requested reference files.
    if args.json or args.markdown or args.all_classes:
        out = args.out or os.path.join(os.getcwd(), "docs", "reference")
...
def write_markdown(path, info, methods, fields):
    ...
    lines = []
    lines.append("# Pivot Animator 5.2.11 — published RTTI catalog")
    lines.append("")
    lines.append("> Generated by `tools/dump_rtti.py`. Do not edit this file by hand.")
    lines.append("")
